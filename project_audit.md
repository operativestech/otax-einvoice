...
|---|--------|---------|----------|
| 1 | **إلغاء / رفض فاتورة** | الوثائق بتوثّق الـ API | **مفيش زرار في الفرونت** للإلغاء أو الرفض. ومفيش endpoint في الباك لإرسال الإلغاء لـ ETA |
| 2 | **رفع دفعة مع تتبع المهام** | رفع Excel شغال (طلب واحد، ينتظر الرد) | **مفيش نظام مهام غير متزامن** — الوثائق بتوصف `/batch/submit` → `/batch/:jobId/status` مع polling للتقدم. حالياً كله متزامن |
| 3 | **تقارير: تحليل الفجوات** | صفحة التقارير موجودة | **مفيش حساب** لمقارنة portal_liability vs erp_liability. ومفيش endpoint لـ `/reports/gap-analysis` |
| 4 | **تقارير: الإحصائيات** | الداشبورد فيه إحصائيات أساسية | **مفيش endpoint إحصائيات منفصل** زي ما في الوثائق (`/reports/statistics` مع فترة، رسوم بيانية) |
| 5 | **المزامنة: سجل وتقدم** | بداية المزامنة + الحالة موجودين | **مفيش جدول سجل** يعرض المزامنات السابقة. ومفيش عرض "آخر مزامنة" |
...
|---|-------------------|----------|--------|
| 1 | **لوحة المطابقة (Reconciliation)** | P1 عالية | الوثائق بتوصف عرض 3 أعمدة (ERP/بنك/ETA)، محرك Auto-Match، مطابقة يدوية، قبول/رفض. **مفيش endpoints في الباك ولا صفحة في الفرونت** |
| 2 | **مزامنة معاملات ERP** | P1 عالية | `/api/otax/transactions/sync` — استيراد AR/AP من نظام ERP. مش موجود |
| 3 | **رفع كشف حساب بنكي** | P1 عالية | `/api/otax/bank/upload` — رفع ملفات CSV للبنك. مش موجود |
| 4 | **محرك المطابقة التلقائي (AI Multi-Pass)** | P1 عالية | خوارزمية Perfect/WHT/FX/Manual matching. مش موجود |
| 5 | **CRUD المطابقات** | P1 عالية | إنشاء/قبول/رفض مطابقات. مش موجود |
| 6 | **تقرير تحليل الفجوات** | P1 عالية | مقارنة مسؤولية البوابة vs مسؤولية ERP. مفيش منطق حساب |
| 7 | **نظام طابور التوقيع الرقمي** | P2 متوسطة | الوثائق بتوصف `/signature/queue`، `/signature/receive`، `/signature/pending`. حالياً التنفيذ مباشر عبر bridge/PFX بدون طابور |
| 8 | **استعلام بيانات دافع الضرائب** | منخفضة | `/api/otax/taxpayer/info/:taxNumber` — مش موجود |
| 9 | **قواعد بيانات AI (4 views)** | منخفضة | `VW_AI_OTAX_SUMMARY`، `VW_AI_OTAX_UNMATCHED`... مفيش SQL views |
| 10 | **تصدير إلى Excel/PDF** | P2 متوسطة | الوثائق بتذكر تصدير التقارير. مش متنفّذ |
...
| **المستندات والفواتير** | 8 | 1 | 0 | **9** |
| **مزامنة البوابة** | 3 | 1 | 0 | **4** |
| **عمليات الدفعات** | 1 | 1 | 0 | **2** |
| **المطابقة (Reconciliation)** | 0 | 0 | 5 | **5** |
| **التقارير والتحليلات** | 1 | 2 | 2 | **5** |
| **التوقيع الإلكتروني** | 3 | 0 | 1 | **4** |
| **أخرى (أكواد، صحة النظام..)** | 7 | 0 | 2 | **9** |
| **الإجمالي** | **33** | **5** | **10** | **48** |

> **نسبة الإنجاز: ~69% خلصان بالكامل، 10% جزئي، 21% ناقص**

---

## حاجات إضافية اتبنت مش في الوثائق (12 ميزة)

الحاجات دي موجودة في المشروع بس **مكتوبتش في أي من الـ 4 وثائق:**

| الميزة | مكانها |
|--------|--------|
| جداول ديناميكية لكل شركة (`org_*_documents`، `org_*_lines`) | `orgTables.ts` |
...
| إدارة Super Admin متعددة المنظمات | `superAdmin.ts`، 4 صفحات Super Admin |
...
1. **أزرار إلغاء / رفض فاتورة** على تفاصيل الفاتورة — سهلة وسريعة
2. **تقرير تحليل الفجوات (Gap Analysis)** — حساب في الباك + رسم بياني في الفرونت
3. **تصدير إلى Excel/PDF** من صفحة التقارير
4. **لوحة المطابقة (Reconciliation) — المرحلة 3** — أكبر ميزة ناقصة، محتاجة باك + فرونت كامل
5. **نظام مهام غير متزامن للدفعات** — حالياً متزامن، محتاج جدول مهام + polling
6. **تحديث الـ 4 وثائق** لتعكس اللي اتبنى فعلاً (معمارية مختلفة، ميزات إضافية)
