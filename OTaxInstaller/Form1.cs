...
namespace OTaxInstaller
{
    public partial class Form1 : Form
    {
        ...
        /// Uses taskkill commands for reliability without needing System.Management
        /// </summary>
        private void StopRunningAgentProcesses()
        {
            ...
            // Kill esbuild.exe — the specific file causing the lock
            killed += KillProcessByName("esbuild");

            // Kill UniversalTokenSigner
            killed += KillProcessByName("UniversalTokenSigner");

            // Use taskkill to force-kill node.exe processes related to OTaxAgent
            // We use WMIC to find node.exe with OTaxAgent in the command line
            try
            {
                var psi = new ProcessStartInfo("cmd.exe",
                    "/c wmic process where \"name='node.exe' and commandline like '%OTaxAgent%'\" call terminate >nul 2>&1")
                {
                    CreateNoWindow = true,
            ...
            // Kill wscript running our VBS via taskkill
            try
            {
                var psi = new ProcessStartInfo("cmd.exe",
                    "/c wmic process where \"name='wscript.exe' and commandline like '%OTaxAgent%'\" call terminate >nul 2>&1")
                {
                    CreateNoWindow = true,
                    UseShellExecute = false
                };
        ...
        /// <summary>
        /// Kills all instances of a process by name. Returns count killed.
        /// </summary>
